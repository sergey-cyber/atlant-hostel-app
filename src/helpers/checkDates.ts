...
const checkRangesOverlaps = (
  datesRangesA: [string, string],
  datesRangesB: [string, string]
): boolean => {
  const convertedDatesA = datesRangesA.map((date) => new Date(date));
  const convertedDatesB = datesRangesB.map((date) => new Date(date));

  return (
    (convertedDatesB[0] >= convertedDatesA[0] && convertedDatesB[0] <= convertedDatesA[1]) || // datesRanges2 start in datesRanges1
    (convertedDatesB[1] >= convertedDatesA[0] && convertedDatesB[1] <= convertedDatesA[1]) || // datesRanges2 end in datesRanges1
    (convertedDatesB[0] <= convertedDatesA[0] && convertedDatesB[1] >= convertedDatesA[1])
  ); // datesRanges2 encloses datesRanges1
};
...
export const checkBookingPeriods = (
  dateRangesArray: DateRangesArrayType,
  selectedDates: [string, string]
) => {
  // Если массив массивов пустой, то проверку не делать
  return !dateRangesArray.length
    ? undefined
    : dateRangesArray.some((array) => {
