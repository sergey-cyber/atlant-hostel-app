...
const getAllReviewsInState = (currentReviewsPart:any, totalReviewsCount:number , averageRating: number):getReviewsPartAction => ({type: GET_ALL_REVIEWS_IN_STATE, currentReviewsPart, totalReviewsCount, averageRating})
...
export const getReviewsPart = (currentRevPart:number, revPartSize:number) => {
    return async (dispatch:any) => { 
        ...
        if(response.resultCode === 0) {
            dispatch(getAllReviewsInState(response.currentReviewsPart,response.totalReviewsCount, response.averageRating));
        } else {(console.log(response.message))} //Если на серваке будет ошибка то прийдет текс ошибки
